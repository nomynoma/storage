...
- `showQuestion()`: 問題表示（単一選択/複数選択/入力問題/テキスト/画像対応）
- `checkAnswer()`: 単一選択の解答チェック
- `toggleChoice()` / `submitMultipleAnswer()`: 複数選択の処理
- `submitInputAnswer()`: 入力問題の解答チェック
...
**選択タイプ別の例：**
- `single`: 選択肢A〜Dから1つ選択
- `multiple`: 選択肢A〜Dから複数選択（正解: A,C など）
- `input`: テキスト入力（選択肢A〜Dは空欄、正解: 数値や文字列）
